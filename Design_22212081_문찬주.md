...
<!--
[이미지 자리] images/design_class_diagram.png
PNG 이미지를 생성할 수 없어 아래 Mermaid 다이어그램으로 대체한다.
필요 시 본 Mermaid 정의를 기반으로 이미지로 export 하여 위 경로에 저장할 것.
-->
...
classDiagram
    ...
    class RouteSearchUI {
        ...
        +showAutoComplete(candidates : List~Station~) void
        +requestRoute(origin : Station, dest : Station) void
    }

    class LocationService {
        -apiClient : ApiClient
        +LocationService()
        +geocode(keyword : String) List~Station~
        +reverseGeocode(lat : double, lng : double) Station
    }

    class RouteCalculator {
        -apiClient : ApiClient
        +RouteCalculator()
        +searchRoutes(origin : Station, dest : Station) List~Route~
        +sortByTransfer(routes : List~Route~) List~Route~
    }
    ...
    class Route {
        ...
        +getSegment(index : int) RouteSegment
    ...
    class GeofenceChecker {
        -preAlertRadius : double
        -finalRadius : double
        +checkDistance(cur : Location, target : Station) double
        +isPreAlert(distance : double) boolean
        +isArrived(distance : double) boolean
    }
    ...
    class StorageManager {
        ...
        +loadCachedRoute() Route
    }

    class ApiClient {
        -baseUrl : String
        -apiKey : String
        +request(endpoint : String, params : Map) String
        +retry(endpoint : String, params : Map) String
    }
    ...
    RouteCalculator --> ApiClient
    RouteCalculator --> Route : creates
    Route "1" *-- "many" RouteSegment
...
<!-- [이미지 자리] images/seq_search_route.png — PNG 미생성, 아래 Mermaid로 대체 -->

```mermaid
sequenceDiagram
    actor User
    participant UI as RouteSearchUI
    ...
    User->>UI: 출발지/도착지 입력
    UI->>LS: geocode(keyword)
    LS->>API: request(geocode, params)
    API-->>LS: 좌표/역 후보 목록
    LS-->>UI: List<Station>
    UI->>UI: showAutoComplete(candidates)
    User->>UI: 역 선택 (origin, dest 확정)
    UI->>RC: searchRoutes(origin, dest)
    RC->>API: request(searchPubTrans, params)
    API-->>RC: 경로 데이터
    RC->>RC: sortByTransfer(routes)
    RC-->>UI: List<Route>
    UI->>ST: saveRecentSearch(station)
    UI-->>User: 추천/대체 경로 표시
```
...
<!-- [이미지 자리] images/seq_start_navigation.png — PNG 미생성, 아래 Mermaid로 대체 -->
...
sequenceDiagram
    ...
    participant LT as LocationTracker
    participant GF as GeofenceChecker

    User->>UI: "이 경로로 안내" 선택
    UI->>NM: new NavigationManager(route)
    NM->>LT: startTracking()
    loop 위치 갱신마다
        LT-->>NM: onLocationUpdate(location)
        NM->>GF: checkDistance(cur, alightStation)
        GF-->>NM: distance
        NM->>NM: updateProgress()
        NM-->>User: 현재 위치/남은 정류장 수 갱신
    end
```
...
<!-- [이미지 자리] images/seq_arrival_alert.png — PNG 미생성, 아래 Mermaid로 대체 -->
...
sequenceDiagram
    ...
    NM->>GF: checkDistance(cur, alightStation)
    GF-->>NM: distance
    alt 한 정거장 전 (isPreAlert == true)
        NM->>AM: showPreAlert(station)
        AM->>AM: vibrate(예고 패턴)
        AM-->>User: 바텀 시트 슬라이드 업 + 예고 진동
    else 목적지 도착 (isArrived == true)
        NM->>AM: showFinalAlert(station)
        AM->>AM: vibrate(연속 패턴)
        AM-->>User: 화면 색상 반전 + 강력한 연속 진동
        User->>AM: 확인 버튼 클릭
    end
```
...
<!-- [이미지 자리] images/seq_auto_update.png — PNG 미생성, 아래 Mermaid로 대체 -->
...
sequenceDiagram
    ...
    Note over NM: 환승 지점 하차 알림 발생 이후
    loop 백그라운드 위치 추적
        LT-->>NM: onLocationUpdate(location)
        NM->>GF: checkDistance(cur, nextBoardingStation)
        GF-->>NM: distance
        alt 다음 노선 반경 내 진입
            NM->>RT: moveToNextSegment()
            RT-->>NM: true (다음 구간)
            NM->>NM: updateProgress()
            NM-->>User: 다음 안내 구간으로 화면/상태 자동 전환
        end
    end
```
...
<!-- [이미지 자리] images/design_state_machine.png — PNG 미생성, 아래 Mermaid로 대체 -->

```mermaid
stateDiagram-v2
    [*] --> RegionSelection
    RegionSelection --> RouteInput : selectRegion()
    RouteInput --> Searching : requestRoute()
    Searching --> RouteInput : 검색 결과 없음
    Searching --> RouteRecommendation : 경로 탐색 성공
    RouteRecommendation --> RouteInput : 다시 검색
    RouteRecommendation --> Navigating : 경로 선택 / startNavigation()
    Navigating --> Navigating : onLocationUpdate() / updateProgress()
    Navigating --> PreAlert : isPreAlert == true
    PreAlert --> Navigating : 확인 / 계속 이동
    Navigating --> AutoTransition : 환승 지점 진입
    AutoTransition --> Navigating : moveToNextSegment()
    Navigating --> FinalAlert : isArrived == true
    PreAlert --> FinalAlert : isArrived == true
    FinalAlert --> [*] : 확인 버튼 / stopNavigation()
```
