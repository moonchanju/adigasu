...
| 2026-06-12 | 0.0.2 | 구현 제약 보완(화면 유지·지나침 보강 안내 반영) | 문찬주 |
...
- 웹/모바일 환경에서 백그라운드 알람이 제한될 수 있다. 이를 완화하기 위해 안내 중에는 화면 잠금(Screen Wake Lock)을 유지하여 화면 꺼짐으로 알림이 멈추는 것을 막는다(미지원 기기는 화면 점유형 알림으로 보완).
- GPS 기반 자동 탑승 감지의 오차가 발생할 수 있다. 도착 반경을 건너뛰고 하차역을 지나친 경우에도 이를 감지해 "다음 정류장에서 내리세요"로 보강 안내한다.
...
- Screen 1: 지역 선택 화면(서비스 가능한 광역시를 선택)
- Screen 2: 출발지와 도착지 입력 화면
- Screen 3: 경로 목록 및 환승 정보 표시 화면
- Screen 4: 실시간 안내 화면(현재 위치, 다음 정류장, 남은 정류장 수 표시)
- Screen 5: 목적지 근접 알림 화면(진동과 함께 시각적 알림 제공)
