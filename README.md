...
| `app/geo.jsx` | Geolocation(`watchPosition`) + Haversine 거리 + 지오펜싱 |
...
- **iOS 진동**: iPhone Safari는 Vibration API 미지원 → 소리(TTS)+화면 색상반전으로 보완됨.
- **지오펜싱 임계값 튜닝**: 도착 70m / 예고 450m(`app/geo.jsx`의 `GEO`)는 실제 탑승 테스트로 보정 권장.
