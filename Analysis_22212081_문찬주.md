...
최근 검색된 역 목록(지역별)과 피드백 설정(음성 안내 on/off, 진동 on/off, 음성 크기 보통/크게, 음성 속도 보통/천천히)을 localStorage에 저장한다. 설정은 지역·입력 화면 헤더의 설정 버튼(바텀 시트)에서 변경할 수 있다. 경로 데이터 캐싱이나 오프라인 모드는 제공하지 않으며, 키가 없거나 호출이 실패하면 대구 지역 데모 데이터로 폴백한다.
